...
var go = module.exports = function startApi (opts, cb) {
  var inited = initApi(opts.customInit);
  ...
  domainMw.events.on('server-error', opts.events.on.bind(opts.events, 'server-error'));
  domainMw.events.on('fatal-error', opts.events.on.bind(opts.events, 'fatal-error'));
